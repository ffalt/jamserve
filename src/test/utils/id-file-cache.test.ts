...
describe('IDFolderCache', () => {
	...
	let cache: IDFolderCache<TestParams>;
	...
	beforeEach(() => {
		...
		cache = new IDFolderCache<TestParams>(
			tempDir.name,
			'test',
			(params: TestParams) => `-${params.size}-${params.format}`
		);
	});
	...
	describe('getExisting', () => {
		...
		test('should return cache result for existing cache file', async () => {
			...
			const cachefile = path.join(tempDir.name, cacheID);
