...
import SubsonicApiSchema from '../../model/subsonic-rest-api.schema.json';
...
export function apiCheck(name: string): express.RequestHandler {
	function CheckApiParametersHandler(req: express.Request, res: express.Response, next: express.NextFunction): void {
		checkOpenApiParameters(name, req, SubsonicOpenApi as OpenAPIObject, SubsonicApiSchema, 'get').then(() => {
			next();
		}).catch((e) => {
