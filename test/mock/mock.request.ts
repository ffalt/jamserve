...
import { OpenAPIObject, OperationObject, ParameterObject, SchemaObject } from '../../src/modules/deco/builder/openapi-helpers.js';
...
function combineData<TName extends string, TData>(list: ReadonlyArray<{ name: TName; data: TData }>): Record<TName, TData> {
	return Object.fromEntries(list.map(({ name, data }) => [name, data])) as Record<TName, TData>;
}
