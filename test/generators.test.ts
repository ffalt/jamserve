...
function downloadZip(req: TestAgent<supertest.Test>, url: string, done: (error?: Error | string) => void): void {
	req.get(url)
		...
		.end((error: Error | undefined, res: { body: Buffer }) => {
			if (error) {
				done(error);
				return;
			}
			if (!(res.body as any)) {
				done(new Error('Invalid Body response'));
				return;
			}
			let isClosedByError = false;
			yauzl.fromBuffer(res.body, (error: Error | null, zipfile?: ZipFile) => {
				if (error) {
					done(error);
					return;
				}
				if (!zipfile) {
					done(new Error('Invalid Zip Buffer'));
					return;
				}
...
function downloadJSON(req: TestAgent<supertest.Test>, url: string, done: (error?: Error | string) => void): void {
	req.get(url)
		.expect(200)
...
function downloadContent(req: TestAgent<supertest.Test>, url: string, contentType: RegExp, done: (error?: Error | string) => void): void {
	req.get(url)
		.expect(200)
