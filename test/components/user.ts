...
export function shouldBehaveLikeAUser() {
	...
	iti('should find and compare the created user by ID', async function() {
		const user = await this.engine.users.getUser(userID);
		should().exist(user);
		expect(user).to.deep.equal(this.user);
	});
	iti('should find and compare the created user by name', async function() {
		const user = await this.engine.store.user.searchOne({name: this.user.name});
		should().exist(user);
		expect(user).to.deep.equal(this.user);
	});
	...
	iti('should auth and compare the created user', async function() {
		const user = await this.engine.users.auth(this.user.name, this.user.pass);
		should().exist(user);
		expect(user).to.deep.equal(this.user);
	});
	iti('should not auth the user with the wrong password', async function() {
		try {
			const user = await this.engine.users.auth(this.user.name, this.user.pass + '_wrong');
	...
	iti('should update the created user', async function() {
		...
		await this.engine.users.updateUser(this.user);
		const user = await this.engine.users.getUser(userID);
		should().exist(user);
		expect(user).to.deep.equal(this.user);
		this.user.name = oldname;
		await this.engine.users.updateUser(this.user);
		const user2 = await this.engine.users.getUser(userID);
